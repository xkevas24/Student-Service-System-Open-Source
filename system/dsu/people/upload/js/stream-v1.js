...
(function(){
	var Provider, aFilters = [], nIdCount = 0, aOtherBrowsers = ["Maxthon", "SE 2.X", "QQBrowser"],
		...
		sTotalContainer = '<div id="#totalContainerId#" class="stream-total-tips">' +
			...
			'	，总大小<strong class="_stream-total-size">&nbsp;</strong>' +
			'</div>',
	...
	Main.prototype = {
		...
		onSelect : function(list) {
			fShowMessage("已选择文件: " + list.length);
		},
		onFileCountExceed : function(selected, limit) {
			fShowMessage("文件数量:" + selected + ", 上限:" + limit, true);
		},
		onMaxSizeExceed : function(file) {
			fShowMessage("文件:" + file.name + "大小是:" + file.size +"溢出大小:" + file.limitSize, true);
		},
		onExtNameMismatch: function(file) {
			fShowMessage("允许上传的文件拓展名: [" + file.filters.toString() + "],文件： 【"+ file.name+"】不被允许上传，建议将其压缩成压缩文件后上传。" , true);
		},
		onAddTask: function(file) {
			fShowMessage("添加至任务 << 文件名: [" + file.name);
		},
		onCancel : function(file) {
			fShowMessage("上传取消: " + file.name);
		},
		onStop : function() {
			fShowMessage("上传停止!");
		},
		onCancelAll : function(numbers) {
			fShowMessage(numbers + " 文件上传被取消！");
		},
		onComplete : function(file) {
			fShowMessage("文件名:" + file.name + ", 文件大小:" + file.size + " 完成情况	[上传成功]");
		},
		onQueueComplete : function(msg) {
			fShowMessage("多文件上传："+msg+"	---==>		[上传成功]");
		},
		onUploadError : function(status, msg) {
			fShowMessage("发生错误.  状态:" + status + ", 错误信息: " + msg, true);
		},
		disable : function() {
