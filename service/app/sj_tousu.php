...
if(isset($_POST["reason"]) && $_POST["reason"]<>""){
	...
	$newid=mysqli_fetch_row(mysqli_query($con,"select max(id) from stu_sj_tousu;"))[0]+1;
	$add_sql="insert into stu_sj_tousu (id,oid,reason) values ('$newid','$oid','$reason');";
	$do=mysqli_query($con,$add_sql);
	if($do){
		//获取管理员学号
		$manager=mysqli_fetch_row(mysqli_query($con,"select stuno from stu_ishop_manager where id='1';"))[0];
		mysqli_close($con);
		$data=shortLink("待处理的商家投诉","学生服务系统",array($manager),"有新的商家投诉，请及时处理！");
