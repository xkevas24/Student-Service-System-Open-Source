...
//更新审核状态
$sql="update stu_jiaoshi_apply set status='已通过' where id='$id';";
$do=mysqli_query($con,$sql);
mysqli_close($con);
if($do){
	echo "<script>alert('操作成功！')</script>";
	//审核通过推送
	$data=shortLink("教室申请通过","学生会学生社团联合会",array($apply[4]),"恭喜您，您提交的".$apply[1]."教室已经通过，使用时间为".$apply[2]."，".$apply[6]."。如有需要请联系".$name.":".$me);
	header("refresh:0;url=../jiaoshi_view.php");
	exit();
}else{
